...
** rclone
:PROPERTIES:
:CREATED:  [2025-08-12 Tue 23:50]
:END:

#+name: bash_alias_rclone
#+begin_src bash :tangle no
# Generic rclone defaults that work well across remotes
alias rclonef='rclone -P \
  --fast-list \
  --checkers 8 --transfers 8 \
  --tpslimit 8 --tpslimit-burst 8 \
  --retries 5 --retries-sleep 5s --low-level-retries 10 \
  --stats 15s \
  --filter-from ~/FilenSync/rclone-filters.txt'
#+end_src
